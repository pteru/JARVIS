#!/bin/bash
# SF Health Monitor — Orchestrator
# Runs the full pipeline: collect -> analyze -> alert with flock-based locking.
# Includes pre-flight checks: campaign flag, connectivity, analysis throttle, heartbeat.
# Usage: run.sh [deployment-id] (default: 02006)
set -euo pipefail

ORCHESTRATOR_HOME="${ORCHESTRATOR_HOME:-$HOME/JARVIS}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEPLOYMENT_ID="${1:-02006}"
...
# ---------------------------------------------------------------------------
LOCK_FILE="/tmp/sf-health-${DEPLOYMENT_ID}.lock"
exec 200>"$LOCK_FILE"
flock -n 200 || {
...
# ===========================================================================
# Gate 2 — Connectivity via ping (single server)
# ===========================================================================
...
if [[ "$REACHABLE_COUNT" -eq 0 ]]; then
    log "Connectivity check FAILED: 0/${TOTAL_NODES} nodes reachable"
    ...
    if [[ "$SEND_VPN_ALERT" == "true" ]]; then
        ...
        CONN_MSG="$(printf '\xF0\x9F\x94\xB4') NETWORK DISCONNECTED — $(jq -r '.name' "$CONFIG_FILE")

Cannot reach SF server from ${HOSTNAME_LOCAL}.
Pipeline aborted — this is NOT a server failure.

Server tried: ${NODE_IPS[*]}
Time: $(date '+%Y-%m-%d %H:%M')
---
JARVIS Health Monitor"

        HTTP_CODE=$(send_telegram "$CONN_MSG")
        if [[ "$HTTP_CODE" == "200" ]]; then
            log "Network disconnect alert sent to Telegram"
        else
    ...
    log "Connectivity check OK: ${REACHABLE_COUNT}/${TOTAL_NODES} nodes reachable"
    # Clear alert state on successful connectivity
    if [[ -f "$VPN_ALERT_STATE" ]]; then
...
# ---------------------------------------------------------------------------
if [[ -d "$ORCHESTRATOR_HOME/data/sf-health/$DEPLOYMENT_ID/$(date -u +%Y-%m-%d)" ]]; then
...
Server healthy. Pipeline running normally.
Time: $(date '+%Y-%m-%d %H:%M')
---
JARVIS Health Monitor"
