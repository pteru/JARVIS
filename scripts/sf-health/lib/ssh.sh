...
if [[ -z "${SF_SSH_PASSWORD:-}" ]]; then
    echo "ERROR: SF_SSH_PASSWORD environment variable is not set" >&2
    exit 1
fi
...
export SSHPASS="$SF_SSH_PASSWORD"
...
node_ssh_port() {
    local node_name="$1"
    jq -r --arg n "$node_name" '.nodes[$n].ssh_port // 22' "$CONFIG_FILE"
}
