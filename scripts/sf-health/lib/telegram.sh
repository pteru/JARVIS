#!/bin/bash
# SF Health Monitor — Telegram notification helper
...
# send_telegram_routed with the given domain (default: "sf-health").
# Otherwise falls back to legacy inline resolution.
# Truncates message to 4000 chars. Returns the HTTP status code.
# ---------------------------------------------------------------------------
send_telegram() {
    local message="$1"
    local domain="${2:-sf-health}"

    # Try router first
    if [[ "$_SF_ROUTER_AVAILABLE" == "true" ]]; then
