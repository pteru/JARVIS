#!/bin/bash
# SF Health Monitor — Threshold alerting with deduplication
# Reads the latest snapshot, checks against thresholds, sends Telegram alerts.
# Usage: alert.sh [deployment-id] (default: 02006)
set -euo pipefail
...
DEPLOYMENT_NAME="${DEPLOYMENT_NAME:-SpotFusion $DEPLOYMENT_ID}"
...
for node in "${NODE_NAMES[@]}"; do
    ...
    if [[ -n "$disk_pct" && "$disk_pct" != "null" ]]; then
        disk_int=${disk_pct%.*}
        if [[ "$disk_int" -ge "$DISK_CRITICAL_PCT" ]]; then
            add_alert "CRITICAL" "${node}_disk_root_critical" "Root disk at ${disk_pct}% on ${node} (threshold: ${DISK_CRITICAL_PCT}%)"
        elif [[ "$disk_int" -ge "$DISK_WARNING_PCT" ]]; then
            add_alert "WARNING" "${node}_disk_root_warning" "Root disk at ${disk_pct}% on ${node} (threshold: ${DISK_WARNING_PCT}%)"
        fi
    fi
