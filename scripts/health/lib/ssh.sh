...
if [[ -z "${HEALTH_SSH_SECRET:-}" ]]; then
    echo "ERROR: HEALTH_SSH_SECRET is not set. Source config.sh and call load_health_config first." >&2
    exit 1
fi

if [[ ! -f "${HEALTH_SSH_SECRET}" ]]; then
    echo "ERROR: SSH secret file not found: $HEALTH_SSH_SECRET" >&2
    exit 1
fi
...
# Load the SSH password into SSHPASS (consumed by sshpass -e)
export SSHPASS
SSHPASS="$(<"$HEALTH_SSH_SECRET")"
...
# ---------------------------------------------------------------------------
# ssh_cmd <node_name> <command> [args...]
...
# Quick SSH connectivity test. Returns 0 if reachable, non-zero otherwise.
# ---------------------------------------------------------------------------
is_node_reachable() {
    local node_name="$1"
