...
- **272 occurrences** of hardcoded credentials found across the codebase (excluding `.venv/` directories):
  - `<sparkeyes-password>` -- Redis password, hardcoded in every `redis_read.py` and `redis_write.py` copy (line 8-9 in each file). Used as the default when no password parameter is provided.
  - `<skm-password>` -- Used for PostgreSQL, RabbitMQ, PgAdmin, and Redis in YAML deployment files.
