...
- `<smartdie-password>` -- Redis password, RabbitMQ password (in services/connect, data-processing, get-data, loader, status, trigger, plc-monitor, firmware-switch, firmware-hub, infra-setup YAMLs, legacy code, toolkit)
- `<skm-password>` -- WiFi password in `firmware/firmware-switch/src/main.cpp`, RabbitMQ in `services/database-writer/.env`, legacy compose files
