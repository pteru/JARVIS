...
| Sandbox Dev Environment | **In Progress** | [spec](../../docs/superpowers/specs/2026-02-20-sandbox-dev-environment.md) |
| Google Drive ↔ PMO Integration | **Planned** | [spec](../../docs/superpowers/specs/2026-02-20-gdrive-pmo-sync.md) |
| Meeting Minutes Cron (GDrive) | **Planned** | [spec](../../docs/superpowers/specs/2026-02-20-meeting-minutes-cron.md) |
| Multi-User Network Interface | **Planned** | [spec](../../docs/superpowers/specs/2026-02-20-multi-user-interface.md) |
| Telegram Command Intake Pipeline | **Planned** | [spec](../../docs/superpowers/specs/2026-02-22-telegram-command-intake.md) |
| Android JARVIS App | **Planned** | [spec](../../docs/superpowers/specs/2026-02-22-android-jarvis-app.md) |
...
| SDK Monorepo Reorganization | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-03-05-sdk-reorganization.md) | Full restructure of SDK into categorized subfolders (libs, tools, infra, third-party, ds, standards, experimental).<br>Consolidate 7 duplicate tool clusters, centralize 7 third-party forks (pylogix ×8, ultralytics ×3, GenICam-SKM, OpENer, lldpd, 3d-gltf, label-studio-ml-backend).<br>Product monorepos consume SDK tools as submodules via `toolkit/`. 5 phases, ~34-47h. |
| Meeting Minutes Cron (GDrive) | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-20-meeting-minutes-cron.md) | Cron-based Google Calendar + Drive scanner for auto-organizing meeting minutes/recordings.<br>AI analysis of transcripts (action items, decisions, follow-ups). Routes to PMO project folders.<br>Builds on google-workspace MCP + meeting-assistant spec. 4 phases, ~10-14h. |
| GWorkspace Tools Package | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-20-gworkspace-tools-package.md) | Package google-workspace MCP server + skills as distributable npm/Docker artifact.<br>Config-driven setup (no hardcoded service accounts). Template credentials, install wizard.<br>3 phases, ~8-12h. |
| JARVIS Distribution — Strokmatic | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-20-jarvis-dist-strokmatic.md) | Non-personal Strokmatic JARVIS distribution for colleagues in `releases/JARVIS-strokmatic/`.<br>Strip personal data (teruel paths, personal emails). Keep product context, skills, MCP servers.<br>Build script with sanitization + config wizard. 4 phases, ~15-20h. |
| JARVIS Distribution — Generic | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-20-jarvis-dist-generic.md) | Fully sanitized JARVIS distribution removing all Strokmatic-specific data.<br>Template-based: generic product examples, placeholder configs, onboarding guide.<br>Depends on Strokmatic distribution first. 3 phases, ~10-15h. |
| Multi-User Network Interface | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-20-multi-user-interface.md) | Web-based multi-user Claude Code/JARVIS gateway accessible over LAN.<br>User auth (local accounts), session isolation, per-user history/storage, activity tracking.<br>Reverse proxy to `claude` CLI processes. Most complex item. 5 phases, ~40-60h. |
| `/cleanup` Skill | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-20-cleanup-skill.md) | Post-task cleanup skill: check worktree + branches, stage/commit changes,<br>update context files + changelog, rerun checks. Standardized end-of-task ritual. ~4-6h. |
| Gmail API Migration | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-03-05-gmail-api-migration.md) | Add Gmail tools to Google Workspace MCP server, migrate email fetch from IMAP to Gmail API.<br>5 new tools (list, read, search, labels, download attachment). Deprecates Python IMAP tool. ~10h. |
| Sandbox Dev Environment | **In Progress** | — | — | [spec](../../docs/superpowers/specs/2026-02-20-sandbox-dev-environment.md) | Docker-based sandbox for autonomous Claude Code task execution.<br>Pre-built base image + disposable per-task containers + git patch review.<br>`--dangerouslySkipPermissions` safe inside container. ~4-6h. |
| Dashboard Modular Sidenav | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-20-dashboard-modules.md) | Restructure orchestrator dashboard into modular sidenav layout.<br>General overview tab + per-tool tabs (PR Review, Health Check, Dispatches, Backlogs, Changelog).<br>Alert badges on tab icons for pending/new actions. 4 phases, ~12-18h. |
| Voice Interface — Session Tracker | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-19-voice-interface.md) | Extension to JARVIS Voice Interface: real-time tracking of active Claude sessions/projects,<br>running status, loaded skills, resource usage. Overlay panel in visual identity. ~6-8h addendum. |
| Telegram Command Intake Pipeline | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-22-telegram-command-intake.md) | Telegram bot → NLP intent classifier → action pipeline router.<br>Bilingual EN/PT-BR. First pipeline: `/backlog-add`. Builds on existing Telegram integration. 5 phases, ~28h. |
| Android JARVIS App | **Planned** | — | — | [spec](../../docs/superpowers/specs/2026-02-22-android-jarvis-app.md) | Android app with "Jarvis" wake word (Porcupine) → on-device STT → JARVIS intake API.<br>Bilingual, dark theme, offline queue. Shares pipeline with Telegram intake. 5 phases, ~44h. |
| Telegram Bot Manager | **Done** | v1.3.0 | 2026-02-22 | [spec](../../docs/superpowers/specs/2026-02-22-telegram-bot-manager.md) | Multi-bot notification routing. Separate bots per domain (alerts, ops, assistant, reports).<br>Config-driven registry, shared router library, backward compatible toggle. 4 phases, ~14h. |
...
| PR Review Service v2 | **Planned** | — | — | [plan](../../docs/superpowers/plans/2026-04-04-pr-review-v2.md) | Smart re-review, auto-post, labels, dashboard, build checks. |
| JARVIS Restructuring | **Planned** | — | — | [plan](../../docs/superpowers/plans/2026-03-23-jarvis-restructuring.md) | Marketplace + shared libs cleanup. |
| Document Templates Plugin | **Planned** | — | — | [plan](../../docs/superpowers/plans/2026-04-04-document-templates-plugin.md) | Strokmatic plugin for report/proposal/presentation templates. |
...
| [voice-pipeline.md](../../docs/superpowers/plans/2026-02-27-voice-pipeline.md) | [voice-pipeline-design.md](../../docs/superpowers/specs/2026-02-27-voice-pipeline-design.md) | Planned |
| [git-graph-viewer.md](../../docs/superpowers/plans/2026-03-11-git-graph-viewer.md) | [git-graph-viewer-design.md](../../docs/superpowers/specs/2026-03-11-git-graph-viewer-design.md) | Planned |
| [pr-review-v2.md](../../docs/superpowers/plans/2026-04-04-pr-review-v2.md) | — | Planned |
| [jarvis-restructuring.md](../../docs/superpowers/plans/2026-03-23-jarvis-restructuring.md) | — | Planned |
| [document-templates-plugin.md](../../docs/superpowers/plans/2026-04-04-document-templates-plugin.md) | — | Planned |
...
| 2026-07-03 | pr-review: failed reviews are never retried | Change detection in `run.sh` is fetch-to-fetch (`pr-inbox.json` vs `.prev`), so a review that fails mid-batch (transient claude error on infra#77 at 20:36) is silently dropped — retried only when *any* PR in the fleet changes. Fix idea: also treat `head_sha ≠ meta.json sha` as changed at the run.sh gate, or persist a retry queue for failed reviews. |
| 2026-07-03 | pr-review: no per-review timeout / size cap | `spotfusion-frontend#94` review ran 2h50m and metered $29.06 API-equivalent (agentic `gh api` looping on a 125-file sprint-sync PR); held the pipeline lock ~4h, blocking 25+ cron ticks. Fix idea: `timeout 600 claude ...` in `review-pr.sh` + treat giant sync/merge PRs (>100 files or >10k lines) as summary-only instead of full agentic review. |

- **GUI-validate-with-playwright pattern/skill**: standardize the "launch Vite dev server + drive with `mcp__playwright__*` (navigate → evaluate on `data-testid`/`dataset.ready`/`window.__*` → screenshot)" loop used to validate the sealer C1 viewer. Screenshot is the render check; `readPixels` is unreliable. Would make autonomous front-end validation repeatable across VK/SF/DM frontends.
