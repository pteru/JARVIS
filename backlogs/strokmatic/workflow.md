...
ClickUp Sprint Task [02] SF-1234
    ...
    │   name includes CU-xxxx or task URL
    │
    ▼
GitHub Issue #45 (created by sync or manually)
    │
    ├── Developer creates branch: feat/CU-xxxx-description
...
### 3.1 ClickUp ↔ GitHub (Built-In)

**Setup:** ClickUp Settings → Integrations → GitHub → Authenticate with OAuth

**What it does automatically:**
- Branch/PR names containing ClickUp task ID (e.g., `CU-abc123`) auto-link to the task
- PR activity appears in ClickUp task activity feed
- ClickUp Automations can trigger on "GitHub PR merged" → move task to target status

**Recommended ClickUp Automation rules:**
- *When* GitHub PR linked to task is merged → *Then* change status to "Complete"
- *When* GitHub branch is created for task → *Then* change status to "In Progress"

**Naming convention for branches:**
```
feat/CU-<task-id>-<short-description>
fix/CU-<task-id>-<short-description>
```
...
7. /clickup CU-xxxx status complete # Close task with deliverable link
```
...
Include the ClickUp task ID in your branch name (e.g., `feat/CU-abc123-fix-camera`). The native integration auto-links them. Alternatively, paste the GitHub Issue URL into the `GitHub Issue` custom field on the ClickUp task.
