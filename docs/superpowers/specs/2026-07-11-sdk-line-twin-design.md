...
- **Parsing**: L5X (XML) → controller, programas, rotinas (rungs RLL em texto neutro), tags, UDTs. CSV de tags do Studio 5000 como fonte complementar/verificação. Parser recursivo próprio para a gramática de rungs (instruções + branches `[ , ]`); lib `l5x` do PyPI apenas como referência.
- **Tag DB tipado**: BOOL/SINT/INT/DINT/REAL, arrays, UDTs com layout de bytes real (necessário para `COP` entre tipos distintos — usado nos assemblies dos drives), aliases, bit-of-word (`Tag.15`, `Buf[2].3`).
...
- **Subset inicial**: `XIC XIO OTE OTL OTU ONS TON TOF RES MOV COP CPS FLL JSR RET EQU NEQ GRT LES GEQ LEQ LIM ADD SUB MUL DIV AND OR NOT BTD CLR NOP AFI`; `MSG` como stub explícito (loga, seta `.DN`).
