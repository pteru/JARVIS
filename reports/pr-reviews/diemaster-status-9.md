...
1. **Credential exposure in `smartdie-status.yml`** — `REDIS_PASSWORD=<smartdie-password>` and `REDIS_PORT=4000` are still hardcoded in the compose file (lines 24-26). This undermines the safe defaults in the Python code. Should use `.env` file or variable substitution (`${REDIS_PASSWORD}`).
