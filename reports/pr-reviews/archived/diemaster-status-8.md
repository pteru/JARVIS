...
**Warnings:**
- **Exposed credential in docker-compose** — The `smartdie-status.yml` file still contains the hardcoded production password `REDIS_PASSWORD=<smartdie-password>` in plain text (line 26). This undermines the security improvements made in the Python code. The compose file should use environment variable substitution or be excluded from version control.
