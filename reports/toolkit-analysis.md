...
**Product-specific concerns:**
- SF uses `<sparkeyes-password>` hardcoded password — must be env-based
...
| Tool | Issue | Severity |
|------|-------|----------|
| DM `parse_mock_data` | Hardcoded Redis password `<smartdie-password>` | Critical |
...
| SF `spotfusion-toolkit/redis-recorder` | Hardcoded Redis password `<sparkeyes-password>` | High |
